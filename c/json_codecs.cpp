...
uint32 u32_json_decode_message(uint32 u32MsgLen, char *sMesg)
{
    ...
    ssMesg = delSpaces(ssMesg);
    
    //ssMesg.erase(remove_if(ssMesg.begin(), ssMesg.end(), isspace), ssMesg.end());
    struct_json* msg = (struct_json*)json_parse_msg(ssMesg);
    string sMsg = json_get_string(msg->get_item_by_name("msg"));
